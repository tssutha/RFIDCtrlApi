...
INT CRFIDCtrlDlg::nCheckMaterialValidityForBoschV2()
{
	gbRFIDTagFound = FALSE;
	gbIsValidWPC = FALSE;
	gszWorkPieceType ="";
	INT nStatus = RFID_NOK;
	gbIsValidWPC = m_rfidCtrl.CheckMaterialValidityForBoschV2(gnNumOfWP);
	if(m_rfidCtrl.GetSearchTagStatus()==FALSE)
	{
		return RFID_TAG_NOT_FOUND;
	}
	gbRFIDTagFound = TRUE; 	
	if(gbIsValidWPC == TRUE)
	{
		
		nStatus = RFID_OK;
		CStringArray szArrMsg;			
		if(m_rfidCtrl.CheckAllPartTypesAreAllowed(gnNumOfWP, gszAllowedPartTypes)==FALSE)			    
		{
			return RFID_PART_NUM_MISMATCH_WITH_PGM;

		}
		else
		{
			if(nWriteBonderIDToRfidTagAndValidateV2(gnMemOffsetForBonderID, gszBonderID, FALSE) == RFID_NOK)
			{
				nStatus = RFID_ID_WRITE_OP_FAILED;
	...
	{
		return RFID_MULTIPLE_PART_TYPES;
	}
...
INT CRFIDCtrlDlg::nCheckMaterialValidityForBosch()
{
	gbRFIDTagFound = FALSE;
	gbIsValidWPC = FALSE;
	gszWorkPieceType ="";
	...
	{
		//if(m_rfidCtrl.SearchForRfidTag() == FALSE)
		//{
		//	return RFID_TAG_NOT_FOUND;
		//}
		//else
		{
			...
			gbIsValidWPC = m_rfidCtrl.CheckMaterialValidityForBosch(gnNumOfWP);
			if(m_rfidCtrl.GetSearchTagStatus()==FALSE)
			{
				return RFID_TAG_NOT_FOUND;
			}
			...
			if(gbIsValidWPC == TRUE)
			{
				gszWorkPieceType = m_rfidCtrl.GetPartTypeInfo(0);
				nStatus = RFID_OK;
				CStringArray szArrMsg;			
				
				/*
				  if(gszAllowedPartTypes.Find(m_rfidCtrl.m_szArrPartType.GetAt(0)) == -1 ||
				   gszAllowedPartTypes.Find(m_rfidCtrl.m_szArrPartType.GetAt(1)) == -1 ||
				   gszAllowedPartTypes.Find(m_rfidCtrl.m_szArrPartType.GetAt(2)) == -1 )
				*/
				if(m_rfidCtrl.CheckAllPartTypesAreAllowed(gnNumOfWP, gszAllowedPartTypes)==FALSE)			    
				{
					return RFID_PART_NUM_MISMATCH_WITH_PGM;
				...
				{
					if(nWriteBonderIDToRfidTagAndValidate(gnMemOffsetForBonderID, gszBonderID, FALSE) == RFID_NOK)
					{
						nStatus = RFID_ID_WRITE_OP_FAILED;
						AfxMessageBox("test failed");
					}
